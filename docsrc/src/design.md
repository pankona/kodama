...
- 複数の Job Queue を扱う場合
  ...
  - 先述の図では、Job Queue は単一のものを使い、インスタンスは増殖しないことを想定した。
  - この場合、Job Queue が死んでしまうとシステム全体として動作しなくなってしまうため、複数 Job Queue (ロードバランサーも) をかまえると冗長性が増して安定すると思われる。
    - 複数の Job Queue を構える場合、積んだ順番にジョブが実行されないという制約が生じることが想定される。
